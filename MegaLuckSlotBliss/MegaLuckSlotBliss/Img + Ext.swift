...
extension UIImage{
    ...
    static func getGIFThumbnail(gifName: String) -> UIImage? {
        // Get the URL for the GIF from the main bundle
        guard let url = Bundle.main.url(forResource: gifName, withExtension: "gif") else {
            return nil
        }
        
        // Create a source for the GIF
        guard let gifSource = CGImageSourceCreateWithURL(url as CFURL, nil) else {
            return nil
        }
        
        // Extract the first frame as a CGImage
        guard let firstFrame = CGImageSourceCreateImageAtIndex(gifSource, 0, nil) else {
            return nil
        }
