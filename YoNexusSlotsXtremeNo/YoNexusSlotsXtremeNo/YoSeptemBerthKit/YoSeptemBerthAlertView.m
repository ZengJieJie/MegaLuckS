...
#import "YoSeptemBerthAlertView.h"
#import "YoSeptemBerthButton.h"

@interface YoSeptemBerthAlertView()
...
@implementation YoSeptemBerthAlertView

+ (void)initialize {
    if (self == [YoSeptemBerthAlertView class]) {
        YoSeptemBerthAlertView *appearance = [self appearance];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        YoSeptemBerthTextField* firstTextField = [[YoSeptemBerthTextField alloc] init];
        ...
        YoSeptemBerthTextField* secureTextField = [[YoSeptemBerthTextField alloc] init];
...
- (void) layoutSubviews {
    ...
    if (CGAffineTransformIsIdentity(self.alertContainer.transform)) {
        ...
        if (self.alertViewStyle == FUIAlertViewStyleLoginAndPasswordInput) {
            [self.textFields[0] setFrame:(CGRect){{0, self.messageLabel.frame.origin.y + self.messageLabel.frame.size.height + 10},{self.alertContentContainer.frame.size.width, 40}}];
            [self.textFields[1] setFrame:(CGRect){{0, ((YoSeptemBerthTextField*)self.textFields[0]).frame.origin.y + ((YoSeptemBerthTextField*)self.textFields[0]).frame.size.height + 5},{self.alertContentContainer.frame.size.width, 40}}];
        }
...
- (CGFloat) totalButtonHeight {
    ...
    [self.buttons enumerateObjectsUsingBlock:^(YoSeptemBerthButton *button, NSUInteger idx, BOOL *stop) {
...
- (NSInteger)addButtonWithTitle:(NSString *)title {
    ...
    YoSeptemBerthButton *button = [[YoSeptemBerthButton alloc] initWithFrame:CGRectMake(0, 0, 250, 44)];
...
- (void) buttonPressed:(YoSeptemBerthButton *)sender {
...
- (YoSeptemBerthTextField *)textFieldAtIndex:(NSInteger)textFieldIndex
{
